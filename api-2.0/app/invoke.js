...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        switch (fcn) {
            case "PlaceQuote":
                // console.log("-----", args);
                // args[0] = "Quote0" + counter;
                //counter++;
                result = await contract.submitTransaction(fcn, args[0]);
                console.log(result);
                result = args[0].toString();
                message = `Successfully placed quote asset`
                break;
            case "RaiseInvoice":
            ...
            case "UpdateQuote":
                ...
                message = `Successfully updated quote asset with key ${args[0]}`
                break;
            case "UpdatePo":
                result = await contract.submitTransaction(fcn, args[0], args[1], args[2], args[3],
                    args[4], args[5], args[6], args[7], args[8]);
            ...
            case "UpdateInvoice":
                ...
                message = `Successfully placed quote asset with key ${args[0]}`
                break;
            case "CreateMasterAgreement":
            case "UpdateMasterAgreement":
                console.log(`Transient data is : ${transientData}`)
                let agrementData = JSON.parse(transientData)
                console.log(`contract data is : ${JSON.stringify(agrementData)}`)
                let key = Object.keys(agrementData)[0]
                const transientDataBuffer = {}
                transientDataBuffer[key] = Buffer.from(JSON.stringify(agrementData.agrement))
                result = await contract.createTransaction(fcn)
                    .setTransient(transientDataBuffer)
                    .submit()
                result = agrementData.agrement
                message = `Successfully submitted transient data`
                break;

            default:
                return `Invocation require either chaincode functions as shared but got wrong function ${fcn}`
        }
